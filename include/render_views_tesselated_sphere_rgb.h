/*
 * render_views_tesselated_sphere_rgb.h
 *
 *  Created on: Dec 23, 2011
 *      Author: aitor
 */

#ifndef RENDER_VIEWS_TESSELATED_SPHERE_RGB_H_
#define RENDER_VIEWS_TESSELATED_SPHERE_RGB_H_
...
namespace pcl
{
	namespace apps
	{
		 ...
		 * \ingroup apps
		 */
		class PCL_EXPORTS RenderViewsTesselatedSphereRGBA
		{
			private:
				std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f> > poses_;
				std::vector<pcl::PointCloud<pcl::PointXYZRGBA>::Ptr> generated_views_;
			...
			public:
				RenderViewsTesselatedSphereRGBA()
				{
					resolution_ = 150;
				 ...
				 */
				void
				generateView(Eigen::Vector3f& cam_position);
				 ...
				 */
				void
				getViews(std::vector<pcl::PointCloud<pcl::PointXYZRGBA>::Ptr>& views)
				{
