/*
 * render_views_tesselated_sphere_rgb.cpp
 *
...
#include <vtkProperty.h> // ========== NEW


void
pcl::apps::RenderViewsTesselatedSphereRGBA::generateView(Eigen::Vector3f& cam_position)
{
	//center object
	double CoM[3];
	...
	first_cam_pos[0] = cam_position[0] * radius_sphere_;
	first_cam_pos[1] = cam_position[1] * radius_sphere_;
	first_cam_pos[2] = cam_position[2] * radius_sphere_;
	std::cout<<"first_cam_pos="<<first_cam_pos[0]<<" "<<first_cam_pos[1]<<" "<<first_cam_pos[2]<<std::endl;
	...
	for (size_t i = 0; i < cam_positions.size(); i++)
	{
		...
		Eigen::Vector3f test = Eigen::Vector3f::UnitY();//UnitY();
		...
		vtkMatrix4x4 * transform_matrix = NULL;
		if(false){
			double roll = 0,pitch = 0, yaw = 20;
			//cam_tmp->Roll(roll);
			//cam_tmp->Pitch(pitch);
			//cam_tmp->Yaw(yaw);
		
		
			vtkSmartPointer<vtkMatrix4x4> transform_identity = vtkSmartPointer<vtkMatrix4x4>::New();		
			transform_identity->Identity();
		
			vtkSmartPointer<vtkTransform> transform_vtk = vtkSmartPointer<vtkTransform>::New();		
			std::cout <<"transform identity="<<*transform_identity<<std::endl;
			transform_vtk->PostMultiply();
			transform_vtk->SetMatrix(transform_identity);		
		
			//transform_vtk->RotateZ(yaw);
			//transform_vtk->RotateX(yaw);
			//transform_vtk->RotateY(yaw);
			std::cout <<"transform vtk="<<*(transform_vtk->GetMatrix())<<std::endl;		
		
		
			transform_matrix = 	cam_tmp->GetViewTransformMatrix();
			std::cout <<"cam transform before="<<*transform_matrix<<std::endl;
			//apply the rotations
			//cam_tmp->ApplyTransform(transform_vtk);
		}
		...
		cam_tmp->Modified();
		transform_matrix = 	cam_tmp->GetViewTransformMatrix();
		std::cout <<"cam transform="<<*transform_matrix<<std::endl;
		...
		transform_matrix = 	cam_tmp->GetViewTransformMatrix();
		std::cout <<"cam transform after modify camera="<<*transform_matrix<<std::endl;
		...
		actor_view->GetProperty()->SetLighting(false); // ========== NEW
		...
		// ========== NEW
		float* color = new float[resolution_ * resolution_ * 4];
		color = render_win->GetRGBAPixelData(0, 0, resolution_ - 1, resolution_ - 1, 1);
		// ========== /NEW
			...
			for (int x = 0; x < resolution_; x++)
			{
				for (int y = 0; y < resolution_; y++)
				{
					...
					else
					{
						...
						// ========== NEW
						cloud->at(y, x).r = color[(y * resolution_ + x) * 4] * 255;
						cloud->at(y, x).g = color[((y * resolution_ + x) * 4) + 1] * 255;
						cloud->at(y, x).b = color[((y * resolution_ + x) * 4) + 2] * 255;
						cloud->at(y, x).a = color[((y * resolution_ + x) * 4) + 3] * 255;
						// ========== /NEW
		...
		else
		{
			...
			for (int x = 0; x < resolution_; x++)
			{
				for (int y = 0; y < resolution_; y++)
				{
					...
					// ========== NEW
					cloud->points[count_valid_depth_pixels].r = color[(y * resolution_ + x) * 4] * 255;
					cloud->points[count_valid_depth_pixels].g = color[((y * resolution_ + x) * 4) + 1] * 255;
					cloud->points[count_valid_depth_pixels].b = color[((y * resolution_ + x) * 4) + 2] * 255;
					cloud->points[count_valid_depth_pixels].a = color[((y * resolution_ + x) * 4) + 3] * 255;
					// ========== /NEW
...
void
pcl::apps::RenderViewsTesselatedSphereRGBA::generateViews()
{
	//center object
	double CoM[3];
	vtkIdType npts_com = 0, *ptIds_com = NULL;
